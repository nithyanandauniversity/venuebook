...
(function(process) {
  (function(global, factory) {
    ...
    function updateDataIs(expr, parent) {
      var tagName = csp_tmpl_1(expr.value, parent),
          conf,
          isVirtual,
          head,
          ref;
    ...
    var RefExpr = {
      ...
      update: function update() {
        ...
        if (this.hasExp) {
          value = csp_tmpl_1(this.rawValue, this.parent);
        }
    ...
    function _each(dom, parent, expr) {
      ...
      if (refExpr) {
        hasRefExpr = csp_tmpl_1.hasExpr(refExpr);
        remRefAttr(dom);
      }
      parentNode.insertBefore(placeholder, dom);
      parentNode.removeChild(dom);
      expr.update = function updateEach() {
        var items = csp_tmpl_1(expr.val, parent),
            frag = createFrag(),
            refAttr = refExpr,
        ...
        if (refAttr && hasRefExpr) {
          refAttr = csp_tmpl_1(refExpr, parent);
        }
        if (refAttr) {
          parent.refs[refAttr] = [];
        }
    ...
    function getTagName(dom, skipDataIs) {
      ...
      return namedTag && !csp_tmpl_1.hasExpr(namedTag) ? namedTag : child ? child.name : dom.tagName.toLowerCase();
    }
    ...
    var settings = Object.create(csp_tmpl_2.settings);
    var util = {
      tmpl: csp_tmpl_1,
      brackets: csp_tmpl_2,
