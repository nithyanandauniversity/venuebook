...
describe('External config file', function() {
  ...
  it('generate the tags using custom pug parser', function(done) {
    cli._cli(['--config', 'test/fixtures/config-parsers-pug']);
    setImmediate(function() {
      expect(test('-e', `${GENERATED_FOLDER}/config-file/parsers-pug.js`)).to.be(true);
      expect(trim(cat(`${GENERATED_FOLDER}/config-file/parsers-pug.js`))).to.be(trim(cat(`${EXPECTED_FOLDER}/config-file/parsers-pug.js`)));
      done();
    });
  });
  after(function() {
