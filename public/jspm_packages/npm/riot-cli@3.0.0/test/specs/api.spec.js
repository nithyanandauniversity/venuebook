...
`);
  ...
  it('make with the right path should not return any error', function() {
    expect(cli.make({from: `${TAGS_FOLDER}/component.tag`}).error).to.be(null);
  });
  ...
  it('make using a missing preprocessor should throw an error', function() {
    ...
    expect(result.error).to.be('The "nope" html preprocessor was not found. Have you registered it?');
  });
